...
def get_keypoint(results, height, width):
    left_sholder_x = int(results.pose_landmarks.landmark[11].x * width)
    left_sholder_y = int(results.pose_landmarks.landmark[11].y * height)
    left_sholder_xy = [left_sholder_x, left_sholder_y]
    ...
    return left_sholder_xy, left_elbow_xy, left_wrist_xy, left_hip_xy, left_knee_xy, left_ankle_xy

def calc_distance(x1, y1, x2, y2, x3, y3): #use bector 
...
def calc_slope(left_shoulder_xy, left_ankle_xy):
    x = [left_sholder_xy[0], left_ankle_xy[0]]
    y = [left_sholder_xy[1], left_ankle_xy[1]]
...
if __name__ == '__main__':
    ...
    cap_file = cv2.VideoCapture('push_up.mp4')
    with mp_pose.Pose(
        ...
        while cap_file.isOpened:
            ...
            if not results.pose_landmarks:
                ...
                left_sholder_xy, left_elbow_xy, left_wrist_xy, left_hip_xy, left_knee_xy, left_ankle_xy = get_keypoint(results,height, width)

                dist_hip = calc_distance(left_sholder_xy[0], left_sholder_xy[1], left_ankle_xy[0], left_ankle_xy[1], left_hip_xy[0], left_hip_xy[1])
                
                dist_knee = calc_distance(left_sholder_xy[0], left_sholder_xy[1], left_ankle_xy[0], left_ankle_xy[1], left_knee_xy[0], left_knee_xy[1])
                
                dist_elbow = calc_distance(left_sholder_xy[0], left_sholder_xy[1], left_wrist_xy[0], left_wrist_xy[1], left_elbow_xy[0], left_elbow_xy[1])
                
                body_slope = calc_slope(left_sholder_xy, left_ankle_xy)
                ...
                cv2.putText(frame, str(int(count)), (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2, cv2.LINE_AA)

                frame = draw_keypoint(frame, RADIUS, CLR_KP, CLR_LINE, THICKNESS, \
